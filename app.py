...
def crear_solucion_matematica_detallada(variante_id, seccion="A", tipo_evaluacion="parcial1"):
    """Versión simplificada para generar soluciones matemáticas"""
    try:
        # Cargar respuestas de la variante
        with open(os.path.join(VARIANTES_FOLDER, f'respuestas_{variante_id}.json'), 'r', encoding='utf-8') as f:
            respuestas = json.load(f)
        ...
        # Título
        doc.add_heading(f'SOLUCIÓN MATEMÁTICA DETALLADA - {tipo_evaluacion.upper()} - SECCIÓN {seccion}', 0)
        doc.add_heading(f'Variante: {variante_id}', 1)
        doc.add_paragraph(f'Fecha de generación: {datetime.now().strftime("%d/%m/%Y")}')
        
        # Soluciones
        doc.add_heading('COEFICIENTE DE GINI', 2)
        p = doc.add_paragraph()
        p.add_run("Datos del problema:").bold = True
        
        gini_data = variante["tercera_serie"][0]
        
        # Crear tabla para los datos
        table = doc.add_table(rows=len(gini_data["ranges"])+1, cols=2)
        table.style = 'Table Grid'
        
        # Encabezados
        table.cell(0, 0).text = "Salario mensual en (Q)"
        table.cell(0, 1).text = "No. De trabajadores"
        
        # Datos
        for i, (rango, trabajadores) in enumerate(zip(gini_data["ranges"], gini_data["workers"]), 1):
            table.cell(i, 0).text = rango
            table.cell(i, 1).text = str(trabajadores)
        
        # Solución
        p = doc.add_paragraph()
        p.add_run("\n\nSolución:").bold = True
        p.add_run(f"\nEl coeficiente de Gini calculado es {respuestas['tercera_serie']['gini']}")
        
        # Guardar archivo
        solucion_filename = f'Solucion_Matematica_{seccion}_{tipo_evaluacion}_{variante_id}.docx'
        doc.save(os.path.join(PLANTILLAS_FOLDER, solucion_filename))
...
def generar_examen():
    try:
        ...
        for i in range(num_variantes):
            ...
            # Generar la solución matemática detallada (versión simplificada)
